...
plt.savefig('all_coefficients_plot_tmdb_popularity.png', bbox_inches='tight')
plt.show()
...
plt.savefig('top_50_coefficients_tmdb_popularity.png', bbox_inches='tight')
plt.close()
...
plt.savefig('bottom_50_coefficients_tmdb_popularity.png', bbox_inches='tight')
plt.close()

regre_ols = sm.OLS( endog=np.array(label, dtype = float), exog = np.array(data_encoded, dtype = float)).fit()
output_file_path = 'regression_results_tmdb_popularity.txt'
